...
def Make_aTlist():
    fr=open("/home/ubuntu/dg1sbot/teacher data.txt","r",encoding='UTF8') # 학번 불러오기
    lines=fr.readlines()
    global Teachers
    for line in lines:
        st = line.split(' ')
...
#/home/ubuntu/dg1sbot/teacher data.txt

def Make_aSellist():
    fr=open("/home/ubuntu/dg1sbot/subject select.txt","r",encoding='UTF8') # 학번 불러오기
    lines=fr.readlines()
...
#/home/ubuntu/dg1sbot/subject select.txt

def Temp_timetable():
    fr=open("/home/ubuntu/dg1sbot/time table data.txt","r",encoding='UTF8') # 학번 불러오기
...
Temp_timetable()
#/home/ubuntu/dg1sbot/time table data.txt
...
def response_link(): # 온라인 시간표 대답 함수
    ...
    stid="none"
    fr=open("/home/ubuntu/dg1sbot/user data.txt","r") # 학번 불러오기
    lines=fr.readlines()
    for line in lines:
        datas=line.split(" ")
...
def Getuseranswerpl(): #유저 인풋 받기
    ...
    if stid=="none":
        ...
        if comp < now:
            ...
            fw = open("/home/ubuntu/dg1sbot/final save.txt", "a")
...
def to_excel():
    ...
    for pg in range(3):                         #엑셀 파일 초기화
        ...
        sheet['A1'] = '=MONTH(TODAY()) & "월" & DAY(TODAY()) & "일      " & "' + str(pg + 1) + "학년" + '자리 이동 명렬"'
        for col in range(4):
            if pg == 0:
                i = col + 0
            elif pg == 1:
            ...
            for row in range(3,len(Name[i])):
                sheet.cell(row, collist[col]).value = None
    lines = fr.readlines()
    for line in lines:
...
def input_stid(): # 학번 입력 함수
    ...
    fr=open("/home/ubuntu/dg1sbot/user data.txt","r") # userdata 저장 및 변경
    lines=fr.readlines()
    fr.close()
    fw=open("/home/ubuntu/dg1sbot/user data.txt","w")
    for line in lines:
        datas=line.split(" ")
        dusid=datas[0]
        if dusid==userid: 
...
def text_editor(): # 원하는 파일 사이트에서 보여주고 편집
    ...
    fr=open("/home/ubuntu/dg1sbot/"+filename+".txt","r")
...
def save_as_file(): # txt file 저장하기
    if request.method=='POST':
        ...
        text=str(text)
        with open("/home/ubuntu/dg1sbot/"+filename+".txt","w",encoding='utf-8') as f:
            f.write(text)
        ...
        fw=open("/home/ubuntu/dg1sbot/log.txt","a")
...
def save_as_xlfile(): # file 저장하기
    if request.method == 'POST':
        ...
        f.save("/home/ubuntu/dg1sbot/"+secure_filename(f.filename))
        return render_template("saved.html")

@application.route('/xldownload',  methods=["GET", "POST"])
def download():
    return send_file("/home/ubuntu/dg1sbot/자리이동 명렬.xlsx", as_attachment=True)
  
@application.route('/dnldfile', methods=['GET','POST'])
def download_file(): # file 다운받기
    if request.method == 'POST':
        filename=request.form['downloadfilename']
        return send_file("/home/ubuntu/dg1sbot/"+filename, attachment_filename=filename, as_attachment=True)

@application.route('/file')
def upload_n_download():
    files=os.listdir("/home/ubuntu/dg1sbot")
    folders=[]
    for file in files:
...
def record_status():
    ...
    for i in range(25):
        ...
        for j in range(13):
            record[i].append('')
        record[i].append(0)
    fr=open("/home/ubuntu/dg1sbot/final save.txt", "r")
    lines=fr.readlines()
    for line in lines:
        if line==lines[0]: continue
        if "none" in line: continue
